...
class Edit():
    ...
    def apply_edit(self, record):
        ...
        updated_sequence += sequence[self.sequence_position + len(self.edit_from):]
    ...
    def remove_edit(self, record):
        sequence = record.seq
        assert(sequence[self.sequence_position:self.sequence_position + len(self.edit_to)] == self.edit_to)
        updated_sequence = sequence[:self.sequence_position] 
        updated_sequence += self.edit_from 
        updated_sequence += sequence[self.sequence_position + len(self.edit_to):]
